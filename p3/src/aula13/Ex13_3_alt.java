...
public class Ex13_3_alt {
	...
	private static Scanner nmr = new Scanner(System.in);
	public static void main(String[] args) {
		do {
			...
			case 6:
				int[] pop = new int[ugeral.size()-1];
				ArrayList<Funcionario> tmp = new ArrayList<>(ugeral);
				int x=0;
				int t=0;
				
				for(Funcionario a : tmp) {
				...
				System.out.println("O Nome mais popular -> "+tmp.get(t));
				break;
			
			case 7:
				System.out.println("Quantos premios pretende sortear? ");
				random(nmr.nextInt());
				break;
	...
	private static void random(int a) {
		...
		if(geral.size()<=a) {
			...
			while(tmp.size()<a) {
				tmp.add((int) Math.random()*geral.size());
			}	
			for(int x: tmp) {
				System.out.println(geral.get(x));
			}
	...
	private static void menu() {
		...
		System.out.println("7 - Sortear bilhetes pelos empregados");
